...
func set[T Comparable[T]](n *node[T], keyIn T) *node[T] {
	if n == nil {
		return makeNode(keyIn, nil, nil)
	}
	cmp := n.key.Cmp(keyIn)
	switch {
	case cmp < 0:
		r1 := set(n.right, keyIn)
	...
	case cmp > 0:
		...
		return balance(n1)
	default:
		return makeNode(keyIn, n.left, n.right)
	}
}

func del[T Comparable[T]](n *node[T], keyIn T) *node[T] {
	if n == nil {
		return nil
	}
	cmp := n.key.Cmp(keyIn)
...
func merge[T Comparable[T]](l *node[T], r *node[T]) *node[T] {
	...
	// merge chooses minimum from right side
